...
def test_id_assigner_assign_new_scene(in_memory_id_assigner, image_as_array):
    scene = create_scene("video_1", image_as_array("frame_0.png"))

    assert scene.scene_id is None
    scene_id = in_memory_id_assigner.assign(scene)

    assert scene.scene_id is not None
    assert scene.scene_id == scene_id


def test_id_assigner_assign_existing_scene(in_memory_id_assigner, image_as_array):
    scene_one = create_scene("video_1", image_as_array("frame_0.png"))
    scene_two = create_scene("video_1", image_as_array("frame_0.png"))

    scene_one_id = in_memory_id_assigner.assign(scene_one)
    scene_two_id = in_memory_id_assigner.assign(scene_two)
...
def test_id_assigner_assign_similar_frame_time_match(
    in_memory_id_assigner, image_as_array, distinct_image, video_start_time, video_end_time
):
...
def test_id_assigner_assign_different_with_time_match(in_memory_id_assigner, image_as_array):
    scene_one = create_scene("video_1", image_as_array("frame_0.png"))
