...
from video_chunker.chunking import calculate_chunk_boundaries
...
class TestCalculateChunkBoundariesEdgeCases:
    ...
    def test_floating_point_precision(self):
        ...
        assert len(result) > 0
        assert all(isinstance(chunk[0], float) and isinstance(chunk[1], float) for chunk in result)
    ...
    def test_chunk_boundaries_are_ordered(self):
        ...
        for i in range(len(result) - 1):
            assert result[i][1] >= result[i][0]  # End time >= start time
            assert result[i][1] <= result[i + 1][1]  # End times are non-decreasing
